...
def index(request):
    ...
    # Display amharic Songs
    songs_amharic = list(Song.objects.filter(language='amharic').values('id'))
    sliced_ids = [each['id'] for each in songs_amharic][:5]
    indexpage_amharic_songs = Song.objects.filter(id__in=sliced_ids)
